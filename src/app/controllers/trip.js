...
class TripController {
  ...
  async createTrip(req, res) {
    ...
    try {
      // Kiểm tra sự tồn tại của bus
      const bus = await Bus.findById(busId).populate('seats');
      if (!bus) {
        return res
          .status(404)
          .json({ message: 'Không tìm thấy bus với ID này' });
      }
      ...
      if (!driver) {
        return res
          .status(404)
          .json({ message: 'Không tìm thấy driver với ID này' });
      }
      ...
      if (!route) {
        return res
          .status(404)
          .json({ message: 'Không tìm thấy route với ID này' });
      }

      // Kiểm tra xem routeId có khớp với route của bus không
      if (bus.route.toString() !== routeId.toString()) {
        return res
          .status(400)
          .json({ message: 'Bus và tuyến đường không phải 1 cặp' });
      }

      // Tạo danh sách các ghế cho trip
      const tripSeats = bus.seats.map((seat) => ({
        seat: seat._id,
        status: false, // Mặc định tất cả ghế chưa được đặt
      }));
  ...
  async bookSeats(req, res) {
    try {
      ...
      const order = {
        ...
        bank_code: '',
        callback_url: `https://6753-2001-ee0-554a-cbf0-d991-e264-7a29-a4da.ngrok-free.app/callback`,
      };
